...
public class GridCommandHandlerMetadataTest extends GridCommandHandlerClusterByClassAbstractTest {
    ...
    public void testMetadataDetails() {
        ...
        checkTypeDetails(log, testOut.toString(), crd.context().marshaller().binaryMarshaller().context().metadata0(bo0.type().typeId()));

        assertEquals(EXIT_CODE_OK, execute("--meta", "details", "--typeId",
            "0x" + Integer.toHexString(crd.context().cacheObjects().typeId("TypeName1"))));
        checkTypeDetails(log, testOut.toString(), crd.context().marshaller().binaryMarshaller().context().metadata0(bo1.type().typeId()));
    ...
    public void testTypeMergedAfterRemoveUpdate() {
        ...
        try {
            ...
            repeat(cnt, i -> {
                ...
                checkTypeDetails(log, testOut.toString(), crd.context().marshaller().binaryMarshaller().context().metadata0(typeIds[i]));
            });
        }
        finally {
            repeat(cnt, i -> {
    ...
    public void testMetadataListDetailsAfterTypeRemoving() throws IOException {
        ...
        try {
            ...
            checkTypeDetails(log, testOut.toString(), crd.context().marshaller().binaryMarshaller().context().metadata0(typeId));
    ...
    /**
     * @param m Binary metadata.
     */
    private void checkTypeDetails(@Nullable IgniteLogger log, String cmdOut, BinaryMetadata m) {
        assertContains(log, cmdOut, "typeId=" + "0x" + Integer.toHexString(m.typeId()).toUpperCase());
        assertContains(log, cmdOut, "typeName=" + m.typeName());
        assertContains(log, cmdOut, "Fields:");

        for (String fldName : m.fields())
            assertContains(log, cmdOut, "name=" + fldName + ", type=" + m.fieldTypeName(fldName));

        for (BinarySchema s : m.schemas())
